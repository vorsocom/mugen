...
class TestMugenAcpFuncAuth(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_provider_helpers_and_jwks(self) -> None:
        services = {
            func_auth.di.EXT_SERVICE_ADMIN_SVC_AUTH: "auth-svc",
        ...
        with patch.object(
            ...
            self.assertEqual(func_auth._auth_provider(), "auth-svc")
    ...
    async def test_session_roles_excludes_handoff_permission_when_not_effective(
        self,
    ) -> None:
    ...
    async def test_user_login_aborts_when_role_resolution_fails(self) -> None:
        user_svc, refresh_svc, jwt_svc, logger = _services()
        registry = _registry(user_svc, refresh_svc)
        user = _user(global_roles=[SimpleNamespace(namespace="com.test", name="admin")])
        user_svc.get_expanded = AsyncMock(return_value=user)
        user_svc.verify_password_hash = Mock(return_value=True)
        auth_svc = SimpleNamespace(
            has_permission_for_any_tenant=AsyncMock(side_effect=SQLAlchemyError("db"))
        )
