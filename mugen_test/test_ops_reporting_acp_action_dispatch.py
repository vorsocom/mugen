...
class _FakeRegistry:
    def __init__(self, svc: _FakeMetricService) -> None:
        self.schema = _FakeSchema()
        self._svc = svc
        self._resource = SimpleNamespace(
            edm_type_name="OPSREPORTING.MetricDefinition",
            service_key="metric_definition_svc",
    ...
    def get_resource(self, entity_set: str):
        if entity_set != "OpsReportingMetricDefinitions":
            raise KeyError(entity_set)
        return self._resource

    def get_edm_service(self, service_key: str):
        if service_key != "metric_definition_svc":
...
class TestOpsReportingAcpActionDispatch(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_dispatch_calls_action_handler_with_validated_payload(self) -> None:
        app = Quart("ops_reporting_action_dispatch_test")

        tenant_id = uuid.uuid4()
        metric_id = uuid.uuid4()
        auth_user = uuid.uuid4()

        fake_service = _FakeMetricService()
