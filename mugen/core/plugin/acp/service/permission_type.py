...
class PermissionTypeService(
    ...
    async def _transition_status(
        ...
        *,
        ...
        if current is None:
            abort(404, "Permission type not found.")

        if current.status != from_status:
            abort(
                409,
                (
                    f"PermissionType can only transition to {to_status} from "
                    f"{from_status}."
                ),
            )

        svc: ICrudServiceWithRowVersion[PermissionTypeDE] = self
        try:
    ...
    async def entity_action_deprecate(
        ...
        """Deprecate an active permission type."""
    ...
    async def entity_action_reactivate(
        ...
        """Reactivate a deprecated permission type."""
        return await self._transition_status(
            entity_id=entity_id,
            expected_row_version=int(data.row_version),
