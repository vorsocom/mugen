...
def create_quart_app(config_name, ipc_queue: asyncio.Queue):
    """Application factory."""
...
def load_config(basedir: str, keyval_storage_gateway: IKeyValStorageGateway) -> None:
    ...
    keyval_storage_gateway.put(
        ...
        os.getenv("MATRIX_CLIENT_PASSWORD"),
    )
    keyval_storage_gateway.put(
        "matrix_client_device_name",
        os.getenv("MATRIX_CLIENT_DEVICE_NAME"),
    )
    keyval_storage_gateway.put(
        "matrix_agent_display_name",
        os.getenv("MATRIX_AGENT_DISPLAY_NAME"),
    )
    ...
    keyval_storage_gateway.put(
        ...
        os.getenv("GROQ_API_CLASSIFICATION_MODEL"),
    )
    keyval_storage_gateway.put(
        "groq_api_completion_model",
        os.getenv("GROQ_API_COMPLETION_MODEL"),
    )
    ...
    keyval_storage_gateway.put(
        "qdrant_endpoint_url",
        os.getenv("QDRANT_ENDPOINT_URL"),
    )


# pylint: disable=too-many-locals
async def run_matrix_assistant(
    basedir: str, log_level: int, ipc_queue: asyncio.Queue
) -> None:
    ...
    async with CustomAsyncClient(
        ...
        #     to_kick = [
        #         x.user_id for x in members.members if x.user_id != client.user_id
        #     ]
