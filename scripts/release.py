...
def _develop_sync_pr_title(version: str) -> str:
    return f"chore(release): sync {version} back to develop"


def _develop_sync_pr_body(version: str) -> str:
    return "\n".join(
        [
            f"Sync released version {version} back into `develop`.",
            "",
...
def _publish_release(args: argparse.Namespace) -> None:
    ...
    merged_develop_pr = _develop_sync_pr(release_branch, state="merged")
    develop_synced = False
    if merged_develop_pr is not None:
        develop_synced = True
        develop_status = f"PR merged: {merged_develop_pr['url']}"
    elif release_branch_available and _is_ancestor(release_branch, "origin/develop"):
        develop_synced = True
        develop_status = "already up to date"
    elif _is_ancestor(merge_commit_oid, "origin/develop"):
        develop_synced = True
        develop_status = "already up to date"
    else:
        open_develop_pr = _develop_sync_pr(release_branch, state="open")
        if open_develop_pr is not None:
            develop_status = f"PR already open: {open_develop_pr['url']}"
        else:
            _ensure_release_branch_ready_for_pr(release_branch)
            develop_pr_url = _run(
                [
                    ...
                    release_branch,
                    "--title",
                    _develop_sync_pr_title(args.version),
                    "--body",
                    _develop_sync_pr_body(args.version),
                ],
                capture=True,
            )
            develop_status = f"PR ready: {develop_pr_url}"

    cleanup_status = "Release branch cleanup: deferred until develop sync is merged."
    if develop_synced:
        local_cleanup = "already absent"
        if _branch_exists_local(release_branch):
        ...
        cleanup_status = f"Release branch cleanup: local={local_cleanup} remote={remote_cleanup}"
    ...
    if not develop_synced:
        print(
            "Next: merge the develop sync PR, then rerun "
            f"`scripts/release.py publish --version {args.version}` to clean up."
        )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    publish = subparsers.add_parser(
        "publish",
        help="Tag the merged release, open a develop sync PR, and clean once synced.",
    )
